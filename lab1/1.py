...
a = 0.2
b = 0.06
polar_r = 2 + sp.sin(12 * t)
polar_phi = 1.8 * t + 0.2 * sp.cos(12 * t)
x = polar_r * sp.cos(polar_phi)
y = polar_r * sp.sin(polar_phi)


def rotate2D(X, Y, angle) -> tuple:
...
def calculateSpeed(x_cord, y_cord) -> tuple:
    Vx = sp.diff(x_cord, t)
    Vy = sp.diff(y_cord, t)
...
def calculateAcceleration(x_cord, y_cord) -> tuple:
    Vx = sp.diff(x_cord, t)
    Vy = sp.diff(y_cord, t)
...
def intoTangentialAndNormal(V, W) -> tuple:
    W_tan = sp.diff(V, t)
...
v_x, v_y = calculateSpeed(x, y)
v = sp.sqrt(v_x * v_x + v_y * v_y)

w_x, w_y = calculateAcceleration(x, y)
w = sp.sqrt(w_x * w_x + w_y * w_y)

w_tan, w_norm = intoTangentialAndNormal(v, w)
...
v_vec = graph.plot([x[0], x[0] + kv * v_x[0]],
                   [y[0], y[0] + kv * v_y[0]],
                   color=[0, 0, 1],
                   label="Speed vector")[0]

w_vec = graph.plot([x[0], x[0] + kw * w_x[0]],
                   [y[0], y[0] + kw * w_y[0]],
                   color=[0, 1, 0],
...
v_rot_x, v_rot_y = rotate2D(x_arr, y_arr, v_alpha[0])
w_rot_x, w_rot_y = rotate2D(x_arr, y_arr, w_alpha[0])

v_arrow = graph.plot(x[0] + kv * v_x[0] + v_rot_x,
                     y[0] + kv * v_y[0] + v_rot_y,
                     color=[0, 0, 1])[0]

w_arrow = graph.plot(x[0] + kw * w_x[0] + w_rot_x,
                     y[0] + kw * w_y[0] + w_rot_y,
                     color=[0, 1, 0])[0]
...
def animation(i) -> list:
    ...
    v_vec.set_data([x[i], x[i] + kv * v_x[i]], [y[i], y[i] + kv * v_y[i]])
    w_vec.set_data([x[i], x[i] + kw * w_x[i]], [y[i], y[i] + kw * w_y[i]])
    ...
    v_rot_x, v_rot_y = rotate2D(x_arr, y_arr, v_alpha[i])
    w_rot_x, w_rot_y = rotate2D(x_arr, y_arr, w_alpha[i])

    v_arrow.set_data(x[i] + kv * v_x[i] + v_rot_x, y[i] + kv * v_y[i] + v_rot_y)
    w_arrow.set_data(x[i] + kw * w_x[i] + w_rot_x, y[i] + kw * w_y[i] + w_rot_y)
